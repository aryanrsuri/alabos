"""
Example heating task implementation for Galactus.
"""
...
from galactus_core.models.task import BaseTask
from galactus_core.models.device import DeviceCapability
        ...
        with self.lab_view.request_resources({Furnace: ["$.inside", 1]}) as (devices, sample_positions):
            ...
            if sample.position != f"{furnace.name}_inside":
                ...
                self.lab_view.move_sample(
                    sample=sample_id,
                    position=f"{furnace.name}_inside"
                )
...
from galactus_core.models.device import Device
...
def create_heating_task_template():
    ...
    template = TaskTemplateCreate(
        ...
        input_schema={
            ...
            "atmosphere": {"type": "string", "required": False, "default": "air", "description": "Atmosphere (air, nitrogen, argon, vacuum)"},
            "heating_rate": {"type": "number", "required": False, "default": 10.0, "description": "Heating rate in °C/min"}
        },
        output_schema={
            "sample_id": {"type": "string", "required": True, "description": "Sample identifier"},
            "target_temperature": {"type": "number", "required": True, "description": "Target temperature in °C"},
            "actual_temperature": {"type": "number", "required": True, "description": "Actual final temperature in °C"},
            "duration": {"type": "integer", "required": True, "description": "Actual duration in minutes"},
            "atmosphere": {"type": "string", "required": True, "description": "Atmosphere used"},
            "final_temperature": {"type": "number", "required": True, "description": "Final temperature in °C"},
            "temperature_profile": {"type": "array", "required": True, "description": "Temperature profile during heating"}
        },
