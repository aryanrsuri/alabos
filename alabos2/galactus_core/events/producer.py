"""Kafka event producer for alabos."""
...
class EventProducer:
    """Kafka event producer for alabos events."""
    ...
    def send_event(
        ...
        try:
            ...
            future = self.producer.send(
                topic=topic, key=key or str(entity_id), value=event.dict()
            )
            ...
            logger.error(
                f"Failed to send event {event_type} for {entity_type} {entity_id}: {e}"
            )
            return False
