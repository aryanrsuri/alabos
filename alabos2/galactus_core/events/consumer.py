"""Kafka event consumer for alabos."""
...
class EventConsumer:
    """Kafka event consumer for alabos events."""
    ...
    def subscribe_to_entity_events(self, entity_type: str, event_types: list[str]):
        ...
        topics = [f"alabos.{entity_type}.{event_type}" for event_type in event_types]
            ...
            if handlers:
                for handler in handlers:
                    try:
                        ...
                        logger.error(
                            f"Error in event handler for {event_type_key}: {e}"
                        )
            else:
